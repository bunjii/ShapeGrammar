...
namespace ShapeGrammar.Classes.Rules
{
    ...
    public class SG_AutoRule060_3D : SG_Rule
    {
        ...
        public override string RuleOperation(ref SG_Shape ss_ref, ref SG_Genotype gt)
        {
            ...
            // element from 3rd rule and from 1st rule

            var initialNodes = new List<SG_Node>();
            var initialElems = ss_ref.Elems.Where(e => e.Autorule == UT.RULE010_MARKER).ToList();
            var initialStuds = ss_ref.Elems.Where(e => e.Autorule == UT.RULE030_MARKER).ToList();
            ...
            for (int i = 0; i < selectedIntGenes.Count; i++)
            {
                ...
                var closestElems = initialElems
                    .OrderBy(e => e.Nodes[1].Pt.DistanceTo(re.Nodes[1].Pt)).ToList();

                
                //for (int j = 0; j < closestElems.Count; j++) 
                //{ 
                //    if (closestElems)
                //}
